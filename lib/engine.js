  ...
  forEach(config.http.server, function(args, block) {
    ...
    forAllArgs(block.listen, function(binding) {
      if (binding == 'default_server') {
        config.http.defaultServer = block;
      } else {
        if (binding.match(/^\d+$/)) {
...
Engine.prototype.route = function(req, res, next) {
  ...
  console.log('default server block:');
  console.dir(config.http.defaultServer);
  console.dir(~servers.indexOf(config.http.defaultServer));
  chosen = chosen[0];
  if (!chosen && config.http.defaultServer && ~getBlocks(servers).indexOf(config.http.defaultServer)) {
    chosen = config.http.defaultServer;
  }
  if (!chosen) next();
  res.writeHead(200, {'Content-Type': 'text/plain'});
  res.end(util.inspect(chosen));
};
