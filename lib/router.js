...
Router.prototype = {
  ...
  proxyRequest: function(done) {
    ...
    //copy request headers to be forwarded
    var reqHeaders = req.allHeaders || {}, keys = Object.keys(reqHeaders);
    for (var i = 0, l = keys.length; i < l; i++) {
      var key = keys[i];
      opts.headers[key] = reqHeaders[key].slice(0);
    }
    //remove headers that are not to be forwarded (host-header is to be regenerated)
    ...
    setHeaders.forEach(function(attrs) {
      ...
      if (attrs.length == 2) {
        ...
        if (val) {
          //TODO: replace or append existing header values?
   ...
   *  if no matching file exists, invoke callback
   */
  serveStaticFile: function(basePath, file, callback) {
    ...
    if (basePath.charAt(0) != '/') {
      basePath = path.join(config.data('base_path'), basePath);
    }
    ...
    if ((err = staticProvider.validateRequest(basePath, pathname))) {
      return res.error(err.reason);
    }
    if (indexFiles && endsWithSlash) {
      files = indexFiles.map(function(file) {
        return path.join(basePath, pathname, file);
      });
    } else {
      files = [path.join(basePath, pathname)];
    }
  ...
  sendHttpError: function(error) {
    ...
    if ((action = errorPages[statusCode])) {
      if (~action.indexOf('://')) {
        ...
        //TODO: does using base_path as root here create a vulnerability?
        var root = chosen.getProperty('root', true) || engine.config.data('base_path');
        //TODO: action might be an absolute path, not falling in root; validation would fail
        //  consider breaking the file serving out into a separate function from that which serves public requests
        this.serveStaticFile(root, action, function(err) {
          console.log('could not serve custom error doc: ' + action);
          helpers.htmlError(req, res, error);
        });
      }
    } else {
      helpers.htmlError(req, res, error);
